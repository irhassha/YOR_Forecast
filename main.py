...
try:
    ...
    if 'SERVICE' in df.columns and 'DAY' in df.columns and 'HOUR' in df.columns:
        ...
        if st.checkbox("Tampilkan detail per HOUR"):
            hour_detail = df.groupby(['SERVICE', 'DAY'])['HOUR'].value_counts(normalize=True).unstack().fillna(0) * 100
            st.markdown("**🕒 Distribusi Persentase Container per HOUR dalam setiap DAY & SERVICE**")
            st.dataframe(hour_detail.style.format("{:.1f}%"))
    else:
        st.info("Kolom SERVICE, DAY, atau HOUR tidak ditemukan di dataset.")
